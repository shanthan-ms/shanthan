...
def extract_publication_info(articles, doctor_name, citation_counts):
    ...
    for article in articles:
        ...
        pub_type_list = [pt.text for pt in article_data.findall("PublicationTypeList/PublicationType")]
...
def process_doctor(record_id, full_name):
    try:
        pmids = fetch_pmids_by_name(full_name)
        if not pmids:
...
if __name__ == "__main__":
    input_file = "your excel file path"
    ...
    if failed_logs:
        ...
        logging.info("🚨 Logged failed/empty searches to failed_searches.xlsx")
